...
export interface PlanLimits { links:number; products:number; services:number; sections:number; customDomain:boolean; analytics:boolean; qrCode:boolean; seoMetadata:boolean }
...
export function canAddSection(planType:BillingPlanCode,currentCount:number){const limit=PLAN_LIMITS[planType].sections;return limit===-1||currentCount<limit}
export function hasFeature(planType:BillingPlanCode,feature:keyof PlanLimits){const value=PLAN_LIMITS[planType][feature];return typeof value==='boolean'?value:value!==0}
export function getRemainingCount(planType:BillingPlanCode,limitType:'links'|'products'|'services'|'sections',currentCount:number){const limit=PLAN_LIMITS[planType][limitType];return limit===-1?null:Math.max(0,limit-currentCount)}
