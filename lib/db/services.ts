...
export async function createService(
  sectionId: string,
  data: Pick<Service, 'name'> &
    ServiceContactMethod &
    Partial<Pick<Service, 'description' | 'price_cents' | 'currency' | 'duration_minutes'>>
...
export async function reorderServices(
  ...
  try {
    ...
    if (results.some(({ error }) => error)) throw new Error('Error reordering services')
    return true
  } catch (error) {
