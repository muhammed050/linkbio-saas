...
import EditorStudio from './editor-studio-pro'
...
export default async function EditPage(){
 ...
 const [{data:socials},{data:products},{data:services}]=await Promise.all([
  supabase.from('social_links').select('*').eq('page_id',page.id).order('position'),
  supabase.from('products').select('*').in('section_id',sections.map((s:any)=>s.id).length?sections.map((s:any)=>s.id):['00000000-0000-0000-0000-000000000000']).order('position'),
  supabase.from('services').select('*').in('section_id',sections.map((s:any)=>s.id).length?sections.map((s:any)=>s.id):['00000000-0000-0000-0000-000000000000']).order('position')
 ]);
 const enriched=sections.map((s:any)=>({...s,links:[]}));
 const {data:links}=await supabase.from('links').select('*').in('section_id',sections.map((s:any)=>s.id).length?sections.map((s:any)=>s.id):['00000000-0000-0000-0000-000000000000']).order('position');
