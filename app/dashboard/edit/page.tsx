...
export default async function EditPage(){
 ...
 const [{data:socials},{data:products},{data:services}]=await Promise.all([
  ...
  supabase.from('products').select('*').in('section_id',filterIds).order('position'),
  supabase.from('services').select('*').in('section_id',filterIds).order('position')
 ]);
 const enriched=sections.map((s:any)=>({...s,links:[]}));
 const {data:links}=await supabase.from('links').select('*').in('section_id',filterIds).order('position');
