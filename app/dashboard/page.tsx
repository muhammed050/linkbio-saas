...
import { PLAN_INFO, type BillingPlanCode } from '@/lib/billing/plans'
...
export default async function DashboardPage() {
  return withAuth(async (user, profile) => {
    ...
    const planCode = profile.plan_type as BillingPlanCode
    ...
    return <div className="dashboard-shell">
      ...
      <main id="main-content" className="dashboard-main">
        ...
        <section className="dashboard-overview-grid" aria-label="ملخص الصفحة والحساب">
          ...
          <article className="dashboard-card dashboard-plan-card"><div className="dashboard-card-heading"><div><p className="dashboard-card-kicker">الباقة الحالية</p><h2>{planNames[planCode]}</h2></div><Crown aria-hidden="true" size={23}/></div><p className="dashboard-card-description">{planInfo.features.slice(0, 2).join(' · ')}</p><Link className="dashboard-text-link" href="/pricing">عرض الباقات <ArrowUpLeft aria-hidden="true" size={16}/></Link></article>
        </section>
